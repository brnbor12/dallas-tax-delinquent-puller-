...
def _build_site_address(row: dict) -> str | None:
    ...
    # Format: "1234 MAIN ST, Houston, TX 77002"
    # parse_address_components expects "STATE ZIP" (space, not comma before zip)
    city_state = f"{city}, TX" if city else "TX"
    if zip_code:
        return f"{street}, {city_state} {zip_code}"
...
class HarrisAbsenteeOwnerScraper(BaseCountyScraper):
    ...
    def _build_record(
        ...
        # HCAD uses ZIP+4 format (e.g. "77251-1562"); truncate to 5-digit zip
        mail_zip = (row.get("mail_zip") or "").strip()[:5]
